...
//  Flappy Bird
//
//  Created by Sebastian Waloszek on 10/05/17.
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func makePipes(){
        ...
        let mouvementAmount = arc4random() % UInt32(self.frame.height/2)
        
        let pipeOffset = CGFloat(mouvementAmount) - self.frame.height / 4
    ...
    func setupGame()  {
        ...
        let moveBGAnimation = SKAction.move(by: CGVector(dx: -bgTexture.size().width, dy: 0.0), duration: 5)
        ...
        while(i < 3){
            ...
            background.position = CGPoint(x: (i * bgTexture.size().width), y: self.frame.midY)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if(contact.bodyA.categoryBitMask == ColliderType.Gap.rawValue || contact.bodyB.categoryBitMask == ColliderType.Gap.rawValue){
           ...
           scoreLabel.text = String(score)
            ...
            gameOverLabel.text = "Game Over! Tap again to play"
